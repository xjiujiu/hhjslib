		...
		import: function(path, callback) {
            ...
            for(var ele in path) {
                switch(this._getFileType(path[ele])) {
                    case 'js': this.importJs(path[ele], callback); break;
                    case 'css': this.importCss(path[ele], callback); break;
        ...
        filterLoadedJs: function(jsPath) {
            ...
            for(var ele in jsPath) {
                for(var item in window.document.scripts) {
                    if(jsPath[ele] === window.document.scripts[item].href) {
                        break;
                    }
                }
                if(jsPath[ele] !== window.document.scripts[item].href) {
        ...
        filterLoadedCss: function(cssPath) {
			...
			for(var ele in cssPath) {
                for(var item in window.document.styleSheets) {
                    if(cssPath[ele] == window.document.styleSheets[item].href) {
                        break;
                    }
                }
                if(cssPath[ele] !== window.document.styleSheets[item].href) {
                    css.push(cssPath[ele]);
                }
			}
